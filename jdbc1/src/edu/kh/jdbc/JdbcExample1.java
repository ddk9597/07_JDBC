...
public class JdbcExample1 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			/* 2-2) DB연결 정보를 이용해서 Connection 객체 생성 */
...
//			System.out.println(conn); // 중간 확인
			...
			// JDBC에서 SQL작성 시 마지막 ;을 작성하면 안된다!!
			// -> "SQL 명령어가 올바르게 종료되지 않았습니다" 오류 발생함
			
			/* 4. Statement 객체 생성 */
			stmt = conn.createStatement();
			// Connection 에 연결된 DB로 sql 전달하고 결과를 받아오는 Statement객체 생성
			
			/* 5. Statement 객체를 이용해서 sql 수행후 결과 반환 받기 */
			rs = stmt.executeQuery(sql);
			
			/* 6. 조회 결과가 담겨있는 ResultSet을 커서를 이용해 1행씩 접근하며 각 행의 컬럼값 얻어오기 */
